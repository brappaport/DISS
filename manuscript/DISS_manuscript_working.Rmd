...
authornote: |
  ...
  3 Icahn School of Medicine, Mount Sinai Hospital, New York City, NY
...
abstract: |
  Maladaptive responses to peer acceptance and rejection arise in numerous psychiatric disorders in adolescence; yet heterogeneity and homogeneity across disorders suggests common and unique mechanisms of impaired social function. We tested the hypothesis that social feedback is processed similarly to other forms of feedback (e.g., monetary) by examining the correspondence between the brain’s response to social acceptance and rejection and behavioral performance on a separate reward and loss task. We also examined the relationship between these brain responses and depression and social anxiety severity. The sample consisted of 113 16–21-year-olds who received virtual peer acceptance/rejection feedback in an event-related potential (ERP) task. We used temporospatial principal component analysis to identify the reward positivity (RewP) and feedback negativity (FN) ERP components. RewP to social acceptance was not significantly related to reward bias nor the FN to social rejection related to loss avoidance, not supporting assumptions about similarities in the processing of social and monetary reward feedback. The relationship between RewP and depression severity, while non-significant, was of a similar magnitude to prior studies. Exploratory analyses yielded a significant relationship between socioeconomic status (SES) and RewP, and between SES and loss avoidance and reward bias. These findings begin to address reverse inference based assumptions of the function of the brain’s response to social feedback, while also suggesting a novel pathway for further study, whereby poverty leads to depression via social and reward learning mechanisms.
...
The current study aims to address the key questions outlined above. Specifically, first we test reverse inference assumptions by analyzing the relationship between individual differences in RewP and FN magnitude to social acceptance and rejection feedback and separate behavioral individual differences of bias towards rewards and away from losses. Testing whether individual or group differences in brain activity are also related to a converging behavioral measure constrains the interpretation of the brain activation [@poldrackCanCognitiveProcesses2006]. Hypothesis 1 then, for this aim, is that individual differences in the RewP to social acceptance will be positively related to behavioral reward bias, and similarly that individual differences in the FN to social rejection will be positively related to behavioral loss avoidance. 
Second, we test diagnostic homogeneity and heterogeneity by analyzing the relationship between individual differences in RewP and FN magnitude to social acceptance and rejection feedback and severity of depression and social anxiety symptoms. Hypothesis 2 is that the RewP to acceptance will be negatively related to depression severity, and that the FN to rejection will be positively related to depression and social anxiety severity. 
Third, we test for the presence of a relationship between behavioral bias towards rewards and away from losses and severity of depression and social anxiety, aiding in the interpretation of the previous analyses. This would provide converging evidence of our proposed nomological network between the ERP, behavioral, and psychopathology measures. Thus, hypothesis 3 is that behavioral reward bias will be negatively related to depression severity, and that behavioral loss avoidance will be positively related to depression and social anxiety severity.
...
### 3.1.1 Association between RewP/FN and reward bias and loss avoidance
...
Structural equation models, modeling all hypotheses simultaneously, supported the pattern of results reported above (Figure S1). The hypothesized model showed good fit according to absolute model fit indices (RMSEA=`r myround(fitm_full[5],2)`, SRMR=`r myround(fitm_full[6],2)`), but it did not show significant improvement in model fit over the simplified null model (Table S1).
...
Neither brain responses to acceptance (RewP) nor rejection (FN) were significantly related to voting behavior. This was true when assessing proportion of all votes cast to keep co-players (RewP: $p$ = `r round(tidy(lm(Keep_prop ~ rewp_r, DISS_do02_full_nooutliers))$p.value[2],2)`; FN: $p$ = `r round(tidy(lm(Keep_prop ~ fn_r, DISS_do02_full_nooutliers))$p.value[2],2)`), and proportion of votes cast to keep co-players that had voted to keep the participant in the previous round of voting/feedback (RewP: $p$ = `r round(tidy(lm(Keep_prop_pr ~ rewp_r, DISS_do02_full_nooutliers))$p.value[2],2)`; FN: $p$ = `r round(tidy(lm(Keep_prop_pr ~ fn_r, DISS_do02_full_nooutliers))$p.value[2],2)`). Furthermore, neither type of voting behavior was not significantly related to other variables of interest, including depression, social anxiety, reward bias, and loss avoidance (see Supplemental Materials).
...
We did not find significant associations between the RewP and behavioral reward bias, and the FN and behavioral loss avoidance. Given the null finding, and thus need for further replication, we caution against interpreting this as definitive evidence that the RewP and FN to social feedback do not use or reflect the same reward and loss related processes shown in monetary reward tasks. One prior study found weak relationships between brain responses to social and monetary rewards (i.e., RewP) in emerging adults ($r$(46) = 0.16, $p$ = 0.27) and early adolescents [$r$(37) = 0.28, $p$ = 0.09; @ethridgeNeuralResponsesSocial2017], and a subset of the current sample showed weak correlations as well [$r$(54) = 0.04, $p$ = 0.76; unpublished data, see @rappaportPeerVictimizationDysfunctional2019 for methods]. However, different structures of social reward tasks have led to a more strongly correlated social and monetary RewP  [@distefanoComparisonElectrocorticalResponse2018; @nelsonNeuralResponseMonetary2021]. Such studies speak well to the domain generality or specificity of the brain's response to social feedback compared to other kinds of feedback. Further studies using convergent behavioral measures are needed to inform the function of these brain signals. For example, behavioral measures of social feedback may better map onto the behavioral measures of reward and loss. In the current sample, reward bias and loss avoidance were weakly correlated to an ERP measure of response to monetary wins (relative to losses; $r$(109) = `r round(corr.test(DISS_do02_full_nooutliers_doors$rewp_doors, DISS_do02_full_nooutliers_doors$Reward_bias_20)$r,2)`, $p$ = `r round(corr.test(DISS_do02_full_nooutliers_doors$rewp_doors, DISS_do02_full_nooutliers_doors$Reward_bias_20)$p,2)`; unpublished data). Another possibility is that cross-measure convergence yields smaller effect sizes than previously considered, and necessitates much larger sample sizes. This suggests that Together these results with help establish the conditions under which brain responses to social and other types of feedback are similar and different.

One potential and intriguing explanation for the lack of correspondence between the RewP/FN and behavioral social reward bias/loss avoidance is that there are a number of psychological processes that occur when one experiences social feedback, with the reward or loss system as just one of those processes. These more complex processes may obscure our ability to detect the “pure” reward- or loss-relevant brain signal. For example, one review proposed that there is a period of attentional orienting to the feedback, followed by the emotional appraisal of the feedback, and finally resulting in emotion regulation [@wangHowBrainReacts2017]. Such theories are supported by the presence of different ERP components identified in the current study (i.e., N1, P2, RewP/FN), and another review supporting the presence of multiple processes in response to rewards [@glazerFRNBroadeningTimecourse2018]. However, such conclusions again rely on reverse inference. Thus, future studies will need to assess the relationship between these brain signals and converging behaviors to better determine their function.

## 4.2 Brain responses to social feedback and depression and social anxiety severity

We did not find support for significant relationships between the RewP and FN and concurrent depression and social anxiety symptoms. One possibility is that blunted RewP and heightened FN responses may reflect risk factors for future depression and/or social anxiety [@kujawaVulnerabilityDepressionYouth2017; @mastenSubgenualAnteriorCingulate2011] rather than indicators of a current depressive or socially anxious state. In fact, prospective studies have found that the RewP interacts with stressful life events to predict later depression [@buraniNeuralResponseRewards2021; @goldsteinStressfulLifeEvents2020; @mackinRewardProcessingFuture2019; @olinoSocialRewardYouth2015; @peggBluntedSocialReward2019]. The clinical literature supports different forms of social rejection (e.g., peer victimization, romantic breakups, parental rejection) as risk factors for depression [@copelandAdultPsychiatricOutcomes2013; @kendlerLifeEventDimensions2003; @monroeLifeEventsDepression1999; @rapeePotentialRoleChildrearing1997; @slavichTargetedRejectionPredicts2009] and social anxiety [@mooreConsequencesBullyingVictimization2017; @pickardNavigatingSocialWorld2018; @reijntjesPeerVictimizationInternalizing2010; @storchPeerVictimizationSocial2005]. Therefore, greater sensitivity (i.e., heightened FN) to rejection, in combination with greater experiences of interpersonal rejection, may lead to depression and/or social anxiety.

## 4.3 Reward bias/loss avoidance and depression and social anxiety severity
