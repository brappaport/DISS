...
```{r}
PILTP_data <- read_excel("/Users/brentrappaport/Box Sync/WashU/Research/DISS/_rawdata/pilt_behavioral/T20/FP_all_T20.xlsx")
piltp_data <- PILTP_data; rm(PILTP_data)

PILTN_data <- read_excel("/Users/brentrappaport/Box Sync/WashU/Research/DISS/_rawdata/pilt_behavioral/T20/FN_all_T20.xlsx")
...
piltp_data_RB1 <-piltp_data_cleaned %>%
  group_by(Subid,Block) %>%
            # For all quantiles
  summarise(Rich_Correct_20 = sum(CurrentTrialRCStatus == "RewardCondition" & CRESP == 1),
            Rich_Incorrect_20 = sum(CurrentTrialRCStatus == "RewardCondition" & CRESP == 0),
            Lean_Correct_20 = sum(CurrentTrialRCStatus == "NotRewardCondition" & CRESP == 1),
            Lean_Incorrect_20 = sum(CurrentTrialRCStatus == "NotRewardCondition" & CRESP == 0),
            ...
            Rich_Correct_20_12quant = sum(CurrentTrialRCStatus == "RewardCondition" & CRESP == 1 & 
                                         Mouth.RT <= piltp_quantile_list[3,1] & Mouth.RT > piltp_quantile_list[1,1]),
            ...
            Rich_Correct_20_123quant = sum(CurrentTrialRCStatus == "RewardCondition" & CRESP == 1 & 
                                         Mouth.RT <= piltp_quantile_list[4,1] & Mouth.RT > piltp_quantile_list[1,1]),
            ...
            Rich_Correct_20_1234quant = sum(CurrentTrialRCStatus == "RewardCondition" & CRESP == 1 & 
                                         Mouth.RT <= piltp_quantile_list[5,1] & Mouth.RT > piltp_quantile_list[1,1]),
...
CLEAN_piltp_data <- piltp_data_RB1 %>%
  mutate(Reward_bias_20 = (log((Rich_Correct_20*Lean_Incorrect_20)/(Rich_Incorrect_20*Lean_Correct_20)))*.5,
         ...
         Disc_20 = (log((Rich_Correct_20*Lean_Correct_20)/(Rich_Incorrect_20*Lean_Incorrect_20)))*.5) %>%
  
  pivot_wider(id_cols="Subid", names_from="Block", values_from=c("Reward_bias_20","Reward_bias_20_12quant","Reward_bias_20_123quant","Reward_bias_20_1234quant","Disc_20"))

CLEAN_piltp_data$Reward_bias_20 <- rowMeans(CLEAN_piltp_data[,c("Reward_bias_20_block1","Reward_bias_20_block2")])
CLEAN_piltp_data$Disc_20 <- rowMeans(CLEAN_piltp_data[,c("Disc_20_block1","Disc_20_block2")])

# Make infinite values NA
is.na(CLEAN_piltp_data) <- sapply(CLEAN_piltp_data, is.infinite)
...
piltn_data_RB1 <-piltn_data_cleaned %>%
  group_by(Subid,Block) %>%
            # For all quantiles
  summarise(Rich_Correct_20 = sum(CurrentTrialRCStatus == "RewardCondition" & CRESP == 0),
            Rich_Incorrect_20 = sum(CurrentTrialRCStatus == "RewardCondition" & CRESP == 1),
            Lean_Correct_20 = sum(CurrentTrialRCStatus == "NotRewardCondition" & CRESP == 0),
            Lean_Incorrect_20 = sum(CurrentTrialRCStatus == "NotRewardCondition" & CRESP == 1),
            N_trials = length(Mouth.RT),
            # For the first two quantiles
...
CLEAN_piltn_data <- piltn_data_RB1 %>%
  ...
  pivot_wider(id_cols="Subid", names_from="Block", values_from=c("Loss_bias_20","Loss_bias_20_12quant","Loss_bias_20_123quant","Loss_bias_20_1234quant","Disc_20"))

CLEAN_piltn_data$Loss_bias_20 <- rowMeans(CLEAN_piltn_data[,c("Loss_bias_20_block1","Loss_bias_20_block2")])
CLEAN_piltn_data$Disc_20 <- rowMeans(CLEAN_piltn_data[,c("Disc_20_block1","Disc_20_block2")])

# Make infinite values NA
is.na(CLEAN_piltn_data) <- sapply(CLEAN_piltn_data, is.infinite)
...
# 4. Merge data
```{r}
DISS_cleaning05_pilt <- merge(CLEAN_piltp_data, CLEAN_piltn_data, by="Subid")
```
