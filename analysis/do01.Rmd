...
DISS_do01 <- DISS_do01_full %>% #subset data to only include those that completed Island Getaway task and have T20age
    ...
    dplyr::select(c(Subid, all_of(measures_list), paste0(factors_list[1:3],"_f")))
```
...
```{r}
ggplot(data=gather(DISS_do02_grandaverage_time[,c("Time","Cz_ga_ACC","Cz_ga_REJ","Cz_ga_resid")], Type, Feedback, -Time), aes(x=Time, y=Feedback, color=Type)) +
  geom_line(size=1.5) +
  ...
  ggsave("/Users/brentrappaport/Box Sync/WashU/Research/DISS/work/figures/Figure_2.png", width = 10.5, height = 6.5)
```
...
vis_miss(DISS_do01[,c(measures_list,factors_list)])
```
...
### 3.1.1. Test MCAR
```{r}
TestMCARNormality(DISS_do01, imputation.number=5, imputation.method="Dist.Free", seed=314)
```

## 3.1.2. Run multiple imputation
```{r}
md.pattern(DISS_do01[,c(1:12)])
DISS_do01.imp <- mice(DISS_do01[,c(1:12)], m = 5, method = 'pmm', seed=314) #generate 5 imputed datasets
...
```{r}
summary(pool(with(data=DISS_do01.imp_z, expr=lm(rewp_r_z ~ Reward_bias_20_z))))
summary(pool(with(data=DISS_do01.imp_z, expr=lm(rewp_r_z ~ Loss_bias_20_z))))

summary(pool(with(data=DISS_do01.imp_z, expr=lm(fn_r_z ~ Reward_bias_20_z))))
summary(pool(with(data=DISS_do01.imp_z, expr=lm(fn_r_z ~ Loss_bias_20_z))))
```

### 5.1.2. Multiple regression (frequentist)
```{r}
...
## 4.2. Aim 2: Relationship between RewP/FN and Depression/Social Anxiety
```{r}
...
## 4.3. Aim 3: Relationship between PILT and Depression/Social Anxiety
```{r}
